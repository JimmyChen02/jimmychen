  ...
  return (
    ...
    >
      ...
      <div
        ...
        style={{ background: "radial-gradient(ellipse 70% 50% at 50% 42%, rgba(245,132,38,0.07) 0%, transparent 70%)" }}
        aria-hidden="true"
      />

      {/* "raw input" label */}
      <motion.div
        className="font-mono text-xs text-white/50 tracking-widest uppercase mb-6 flex items-center gap-3"
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
      ...
      >
        <div className="rounded-xl border border-white/10 overflow-hidden shadow-[0_0_40px_rgba(245,132,38,0.06)]">
          {/* Title bar */}
          <div className="flex items-center gap-1.5 px-4 py-2.5 bg-white/3 border-b border-white/8">
            <span className="w-2.5 h-2.5 rounded-full bg-[#ff5f57]" />
            <span className="w-2.5 h-2.5 rounded-full bg-[#febc2e]" />
            <span className="w-2.5 h-2.5 rounded-full bg-[#28c840]" />
            <span className="font-mono text-xs text-white/45 ml-2">python · transformers 4.40</span>
          </div>
          ...
          <div className="px-5 py-5 bg-white/[0.02] text-left space-y-2">
            ...
            <p className="font-mono text-xs leading-relaxed">
              <span className="text-knicks-orange/50">&gt;&gt;&gt; </span>
              <span className="text-knicks-blue/70">from</span>
              <span className="text-white/40"> transformers </span>
              <span className="text-knicks-blue/70">import</span>
            ...
            <p className="font-mono text-xs leading-relaxed">
              ...
              <span className="text-knicks-orange/60">&apos;bert-base-uncased&apos;</span>
            ...
            <p className="font-mono text-xs leading-relaxed">
              ...
              <span className="text-knicks-orange/60">&quot;</span>
              {/* ── The name — large and bright ── */}
              <span className="text-white text-lg font-bold tracking-wide">{displayed}</span>
              <span
                className={cn(
                  "inline-block w-[2px] h-[18px] bg-knicks-orange ml-0.5 align-middle",
            ...
            <AnimatePresence mode="wait">
              {isDone && (
                <motion.p
                  key="status"
                  className="font-mono text-xs text-knicks-orange/50 pt-1"
                  initial={{ opacity: 0 }}
                  animate={{ opacity: 1 }}
                  exit={{ opacity: 0 }}
        ...
        {siteConfig.hero.ctaButtons.map((btn) => {
          ...
          if (btn.variant === "primary") return (
            <a key={btn.label} href={btn.href}
              ...
              className="flex items-center gap-2 px-5 py-2.5 rounded-lg bg-knicks-orange text-black font-semibold text-sm hover:opacity-90 transition-opacity"
            >{icon}{btn.label}</a>
          );
          ...
          if (btn.variant === "secondary") return (
            <a key={btn.label} href={btn.href}
              {...(isExternal ? { target: "_blank", rel: "noopener noreferrer" } : {})}
              className="flex items-center gap-2 px-5 py-2.5 rounded-lg border border-knicks-orange/30 text-knicks-orange text-sm hover:bg-knicks-orange/8 transition-colors"
            >{icon}{btn.label}</a>
          );
      ...
      >
        <p className="font-mono text-xs text-white/45 tracking-widest">scroll to process</p>
        <motion.div
          animate={{ y: [0, 5, 0] }}
